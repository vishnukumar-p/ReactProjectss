...
class App extends Component {
  ...
  onDeleteButton = id => {
    const newList = initialTracksList.filter(each => each.id !== id)
    this.setState({songsList: newList})
  }
      ...
      <AppContainer>
        <BannerContainer>
          <TextContainer>
            ...
            <ParaElement>Singer</ParaElement>
          </TextContainer>
        </BannerContainer>
        <InputAndListContainer>
          <HeaderAndInput>
            ...
            <ListContainer>
              {filteredList.map(each => (
                <ListItem key={each.id}>
                  <FirstContainer>
                    <SongImage src={each.imageUrl} alt="track" />
                    <TextContainer>
                  ...
                  <LastContainer>
                    <Time>{each.duration}</Time>
                    <DeleteButton
                      type="button"
                      data-testid="delete"
                      onClick={() => this.onDeleteButton(each.id)}
                    >
                      <MdDelete size={35} />
                    </DeleteButton>
                  </LastContainer>
